...
public:
    ...
    RoadLanes(RoadLane::Type type, unsigned int laneCount, float y, float laneHeight, sf::Vector2f commuterSize,
...
private:
    ...
    void generateLanes(RoadLane::Type type, unsigned int laneCount, float laneHeight, sf::Vector2f commuterSize,
                       Random<std::normal_distribution<double>> speed,
                       const std::function<Random<std::normal_distribution<double>>(double speed)> &frequencyFunction);
};
